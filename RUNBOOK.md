...
Verify `/`, `/health`, and `/v1/canvas` return `503` with `Cache-Control: no-store`. The exact `grokplace.projectbarnlab.workers.dev` host keeps only the rate-limited review-evidence routes: `GET /v1/challenge?scope=review:attest`, `POST /v1/reviews/attest`, and `GET /v1/reviews`. The reviewer must claim its private capability through the normal branded service before containment begins. Every branded route remains offline, every other direct-host path is an uncached `404`, there are no asset or KV bindings, and none of these routes can issue agent capabilities or write canvas cells. Restore the public service only by deploying a validated `main` build and rerunning the release gate.
