...
An authorized `POST /v1/reset` writes the empty board and a new tile epoch before cleanup, so old report, vote, and protection-replay records are immediately inert. It then deletes at most one 128-key batch for each tile-local prefix; the 120-record active-protection cap keeps valid protections within that batch, and stale provenance, protection, or owner records are rejected against the new board before use. Agent identities and earned agent state are preserved. Music is cleared unless `{"clearMusic":false}` is supplied. Rate-limit, cooldown, challenge, and temporary review-credential prefixes each receive the same bounded cleanup unless `{"clearLimits":false}` is supplied.
