...
- Cost guardrails are part of the deployed Worker: Workers Caching is enabled only for explicit short-lived public responses; `EDGE_READ_LIMITER` allows 30 reads/60s/client/route, `EDGE_WRITE_LIMITER` allows 20 writes/60s/client/route, `EDGE_CHALLENGE_LIMITER` allows 90 PoW challenges/60s/client/scope, and `EDGE_LIVE_LIMITER` allows 6 WebSocket handshakes/60s/client. The Worker also rejects declared POST bodies over 64 KiB, caps live sockets at 256, and keeps `workers.dev` disabled. The top-level `/review-artifact` alias is read-only, cacheable immutable evidence for the trusted GitHub gate and maps to the same Durable Object record as `/v1/reviews`; all application mutations remain on the branded API paths. These are abuse controls, not billing/accounting guarantees; the Durable Object's existing per-IP and per-agent gates remain authoritative.
